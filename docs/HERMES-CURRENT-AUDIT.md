...
| Rich transcript parity | Partial on the open stack described above: markdown, tables, code, quiet/advanced tools, working avatar, transcript actions, jump-to-bottom, local transcript find, exact-source message-level child creation, bounded typed tool-history hydration, file-edit diffs, structured terminal output with bounded ANSI styling, web-search result cards, exact successful generated-image cards/lightbox/share, and explicit assistant `MEDIA:` image/audio/video/file cards exist there. The separate explicit text-only durable Queue has source/session-qualified persistence, edit/pause/remove/uncertainty semantics, and focused automated coverage. Assistant `MEDIA:` loads are all explicit and source-fenced. Generated public-URL images also require explicit contact, while an admitted gateway-path generated image starts its bounded authenticated load when its card appears. Gateway assistant-media images/files use managed reads, audio/video use the stream route, and public URLs use bounded no-credential/no-redirect loading. Live gateway/device proof remains open. Persistent multimodal message parts, specialist math/diagram renderers, per-message TTS, whole-turn timing, the current-chat client-local N/M response picker, and tour/activity remain. Durable session-tree presentation is additionally blocked upstream: JSON-RPC `session.list` drops all parent/root fields and needs a normalized branch-parent root so later compression cannot orphan a real child. Desktop N/M selection changes only local renderer visibility; it sends no gateway RPC and cannot be reconstructed from a normal resume. Durable `session.branch` child creation is already implemented and must not be conflated with that local alternative group. |
