...
| `session.events.since {session_id,last_seen}` returns bare event objects plus `latest_seq`, `truncated`, `count`, and `epoch`. | **Implemented with strict bounded decoding.** Session identity, epoch, count, cursor, bare-object shape, contiguous order, and a 512-event/session plus 2,048-event/transaction ceiling are enforced. RPCs have a five-second mobile deadline and retain at most the server's exact 64-session ring bound. |
