...
| Markdown transcript, actions, tools, media, branching | Native transcript with markdown/tables/code, quiet and advanced tools, edit/rewind/regenerate, exact-source child creation, typed retained-history hydration, bounded diff/structured-terminal-with-ANSI/web-search renderers, local find, generated-image cards/lightbox/share, and explicit assistant `MEDIA:` image/audio/video/file cards | Partial / open | Live/device certification remains open. Persistent multimodal message parts, grouped generic tool runs, specialist math/diagram rendering, the current-chat client-local N/M response picker, per-message TTS, whole-turn timing, tours, and transcript-wide activity remain unimplemented. Durable session-tree presentation is blocked until Hermes `session.list` exposes normalized branch-parent lineage; the current RPC drops parent/root metadata and cannot represent a branch whose parent compression tip rotates again. |
...
session lineage/timing surfaces,
...
- [ ] Finish rich transcripts and full session interaction controls.
      ...
      current-chat client-local N/M response picker and the separate durable
      session-tree lineage presentation, which is blocked on a normalized
      lineage contract from Hermes `session.list`, persistent
