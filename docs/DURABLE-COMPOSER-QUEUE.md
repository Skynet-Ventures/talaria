...
characters, and a 2 MB persisted envelope. A source session drains in FIFO
order; at most two unrelated source sessions drain concurrently. The drain
resumes the exact stored session and sends only after that exact session reports
idle with no in-flight turn, approval, or clarify state. It holds the exact
pooled connection and profile-traffic leases across that resume/submit boundary,
so a managed-cloud replacement or profile lifecycle fence cannot retarget the
wire call. A source/client or receipt ambiguity becomes `uncertain` and is
never replayed.
...
- `retry-exhausted` has spent its persisted automatic retry budget. Relaunch
  and reconnect do not reset it or make it replayable again.
...
protected-file read does not overwrite the original envelope.
...
Hermes does not expose a client idempotency key, prompt-submit lookup, or
client-authoritative cancel for a lost receipt. Those are protocol limits, so
Talaria deliberately records uncertainty rather than trying text-based replay.
