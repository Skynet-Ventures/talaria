...
final class DurableComposerQueueTests: XCTestCase {
    ...
    func testReplaceRollsBackWhenPersistenceFails() throws {
        let (store, url) = makeStore()
        defer { try? FileManager.default.removeItem(at: url) }
        ...
        XCTAssertEqual(store.entry(id: entry.id)?.text, "before")
    ...
    func testExactEditReservationBlocksFIFOHeadAndRejectsWrongRoute() throws {
        let (store, url) = makeStore()
        defer { try? FileManager.default.removeItem(at: url) }
        ...
        XCTAssertEqual(store.entry(id: first.id)?.text, "first")
        model.chat(for: botID).storedSessionID = "stored"
        XCTAssertNotNil(model.beginEditingDurableQueuedPrompt(id: first.id, botID: botID))
